            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                        <p class="text-slate-400 dark:text-slate-400 text-xs font-semibold uppercase tracking-wider">Active Subscriptions</p>
                        <h4 class="text-3xl font-extrabold mt-1 text-indigo-600 dark:text-indigo-400">
                            {{ stats.active_count }}
                        </h4>
                        <span class="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Clients currently paying fee</span>
                    </div>

                    <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                        <p class="text-slate-400 dark:text-slate-400 text-xs font-semibold uppercase tracking-wider">Monthly Recurring Revenue (MRR)</p>
                        <h4 class="text-3xl font-extrabold mt-1 text-emerald-600 dark:text-emerald-400">
                            {{ formatCurrency(stats.mrr) }}
                        </h4>
                        <span class="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Combined monthly SaaS earnings</span>
                    </div>

                    <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                        <p class="text-slate-400 dark:text-slate-400 text-xs font-semibold uppercase tracking-wider">Annual Recurring Revenue (ARR)</p>
                        <h4 class="text-3xl font-extrabold mt-1 text-emerald-600 dark:text-emerald-400">
                            {{ formatCurrency(stats.arr) }}
                        </h4>
                        <span class="text-[10px] text-slate-400 dark:text-slate-500 font-medium">MRR extrapolated yearly</span>
                    </div>

                    <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                        <p class="text-slate-400 dark:text-slate-400 text-xs font-semibold uppercase tracking-wider">Renewals Due</p>
                        <h4 class="text-3xl font-extrabold mt-1" :class="stats.due_soon_count > 0 ? 'text-amber-500 animate-pulse' : 'text-slate-500 dark:text-slate-350'">
                            {{ stats.due_soon_count }}
                        </h4>
                        <span class="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Renewals due in next 7 days</span>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-md border border-slate-100 dark:border-slate-700 p-6">
                    ...
                    <div v-if="licenses.length === 0" class="py-16 text-center">
                        ...
                        <p class="text-slate-500 dark:text-slate-400 font-medium">No license agreements configured.</p>
                        <p class="text-slate-400 dark:text-slate-500 text-xs mt-1">Register a client first, then configure their license to start tracking subscriptions.</p>
                    </div>
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-700/60 text-sm">
                                ...
                                >
                                    ...
                                    <td class="py-3.5 px-4 text-center whitespace-nowrap">
                                        <span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-slate-100 dark:bg-slate-750 text-slate-600 dark:text-slate-300 border border-slate-200/50 dark:border-slate-700">
                                            {{ lic.billing_cycle }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="py-3.5 px-4 text-center whitespace-nowrap">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <button 
                                                ...
                                                class="p-1.5 hover:bg-rose-50 dark:hover:bg-rose-950/30 text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 rounded-lg transition"
                                                title="Delete"
                                            >
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-md border border-slate-100 dark:border-slate-700 p-6">
                    ...
                    <div v-if="clients.length === 0" class="py-6 text-center text-slate-400 dark:text-slate-500 italic text-xs font-semibold">
                    ...
                    <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div 
                            v-for="cl in clients" 
                            :key="cl.id"
                            class="p-4 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-850/20 flex flex-col justify-between"
                        >
                            <div>
                                <h4 class="font-extrabold text-slate-800 dark:text-slate-100">{{ cl.name }}</h4>
                                <p class="text-xs text-slate-450 dark:text-slate-400 mt-0.5">{{ cl.email || 'No email provided' }}</p>
                                <span class="inline-block px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-50 border border-indigo-150 text-indigo-700 dark:bg-slate-900 dark:border-slate-850 dark:text-indigo-400 mt-3">
                                    SaaS: {{ cl.saas_name }}
                                </span>
                            </div>
                            <div class="flex items-center justify-end mt-4 pt-3 border-t border-slate-100 dark:border-slate-700">
                                <button 
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-md border border-slate-100 dark:border-slate-700 p-6">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                        <div>
                            ...
                            <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">Audit log of all subscription payments collected from clients</p>
                        </div>
                        <div class="flex flex-col sm:flex-row gap-3">
                            ...
                            <div class="w-full sm:w-48">
                                <select 
                                    ...
                                    class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                >
                            ...
                            <div class="relative w-full sm:w-64">
                                <input 
                                    ...
                                    class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl pl-9 pr-4 py-2 text-xs font-semibold text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                />
                    ...
                    <div v-if="filteredPayments.length === 0" class="py-12 text-center">
                        <div class="bg-slate-50 dark:bg-slate-900 w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                        </div>
                        <p class="text-slate-500 dark:text-slate-400 text-sm font-medium">No payments found</p>
                        <p class="text-slate-400 dark:text-slate-500 text-xs mt-0.5">Payments appear here when you click "Log Payment" on active licenses.</p>
                    </div>

                    <div v-else class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="border-b border-slate-100 dark:border-slate-700 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-700/60 text-xs">
                                ...
                                >
                                    ...
                                    <td class="py-3 px-4 uppercase font-bold text-[10px]">
                                        <span class="px-2 py-0.5 rounded bg-indigo-50 dark:bg-indigo-950/20 text-indigo-650 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/30">
                                            {{ payment.billing_cycle }}
                                        </span>
                                    </td>
                                    <td class="py-3 px-4 text-slate-550 dark:text-slate-400 font-medium">
                                        {{ payment.description }}
                                    </td>
                ...
                <div class="relative flex flex-col w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl outline-none focus:outline-none">
                    ...
                    <form @submit.prevent="submitClient">
                        <div class="p-6 space-y-4">
                            <!-- Client Name -->
                            <div>
                                <label for="client-name" class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Client Name</label>
                                <input 
                                    type="text" 
                                    id="client-name" 
                            ...
                            <!-- Client Email -->
                            <div>
                                <label for="client-email" class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Client Email</label>
                                <input 
                                    type="email" 
                                    id="client-email" 
                            ...
                            <!-- SaaS Product Name -->
                            <div>
                                <label for="saas-name" class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">SaaS Product Name</label>
                                <input 
                                    type="text" 
                                    id="saas-name" 
                        ...
                        <div class="flex items-center justify-end p-6 border-t border-slate-100 dark:border-slate-700 rounded-b gap-3">
                            <button type="button" @click="showClientModal = false" class="px-4 py-2 text-slate-500 dark:text-slate-400 font-semibold text-sm hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition">
                                Cancel
                            </button>
                ...
                <div class="relative flex flex-col w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl outline-none focus:outline-none">
                    ...
                    <form @submit.prevent="submitLicense">
                        <div class="p-6 space-y-4">
                            <!-- Client Dropdown -->
                            <div>
                                <label for="license-client" class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Select Client</label>
                                <select 
                                    id="license-client" 
                            ...
                            <!-- Amount -->
                            <div>
                                <label for="license-amount" class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">License Fee ($)</label>
                                <input 
                                    type="number" 
                                    step="0.01" 
                                    id="license-amount" 
                            ...
                            <!-- Billing Cycle -->
                            <div>
                                <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Billing Cycle</label>
                                <div class="grid grid-cols-2 gap-4">
                                    <button 
                                        ...
                                        :class="licenseForm.billing_cycle === 'monthly' 
                                            ? 'bg-indigo-50 border-indigo-500 text-indigo-700 dark:bg-slate-900 dark:border-indigo-500 dark:text-indigo-400' 
                            ...
                            <!-- Next Renewal Date -->
                            <div>
                                <label for="license-renewal" class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">First Renewal Date</label>
                                <input 
                                    type="date" 
                                    id="license-renewal" 
                            ...
                            <!-- Status -->
                            <div>
                                <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">License Status</label>
                                <div class="grid grid-cols-2 gap-4">
                                    <button 
                                        type="button"
                        ...
                        <div class="flex items-center justify-end p-6 border-t border-slate-100 dark:border-slate-700 rounded-b gap-3">
                            <button type="button" @click="showLicenseModal = false" class="px-4 py-2 text-slate-500 dark:text-slate-400 font-semibold text-sm hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition">
                                Cancel
                            </button>
                            <button type="submit" class="px-6 py-2.5 bg-indigo-600 dark:bg-indigo-650 hover:bg-indigo-700 dark:hover:bg-indigo-600 text-white font-bold text-sm rounded-xl shadow-md transition" :disabled="licenseForm.processing">
                                {{ licenseForm.processing ? 'Configuring...' : 'Configure' }}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="relative flex flex-col w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl outline-none focus:outline-none">
                    <div class="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-700 rounded-t">
                        <div>
                            ...
                            <p v-if="selectedLicenseForPayment" class="text-xs text-slate-450 dark:text-slate-400 mt-1">
                    ...
                    <form @submit.prevent="submitPayment">
                        <div class="p-6 space-y-4">
                            <!-- Amount -->
                            <div>
                                <label for="payment-amount" class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Amount Received ($)</label>
                                <input 
                                    type="number" 
                                    step="0.01" 
                                    id="payment-amount" 
                            ...
                            <!-- Account Select -->
                            <div>
                                <label for="payment-account" class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Deposit Account</label>
                                <select 
                                    id="payment-account" 
                            ...
                            <!-- Transaction Date -->
                            <div>
                                <label for="payment-date" class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Payment Date</label>
                                <input 
                                    type="date" 
                                    id="payment-date" 
                            ...
                            <!-- Description -->
                            <div>
                                <label for="payment-desc" class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Description / Audit Note</label>
                                <textarea 
                                    id="payment-desc" 
                            ...
                            <div class="flex items-start mt-2">
                                <div class="flex items-center h-5">
                                    <input 
                                        ...
                                        class="h-4.5 w-4.5 rounded border-slate-300 dark:border-slate-700 text-indigo-650 focus:ring-indigo-500 bg-slate-50 dark:bg-slate-900"
                                    />
                                </div>
                                <div class="ms-3 text-xs">
                                    ...
                                    <p class="text-slate-400 dark:text-slate-500 font-medium">Check this if this payment completes the billing cycle and you want to advance the next renewal date.</p>
                                </div>
                            </div>
                        </div>

                        <div class="flex items-center justify-end p-6 border-t border-slate-100 dark:border-slate-700 rounded-b gap-3">
                            <button type="button" @click="showPaymentModal = false" class="px-4 py-2 text-slate-500 dark:text-slate-400 font-semibold text-sm hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition">
                                Cancel
                            </button>
